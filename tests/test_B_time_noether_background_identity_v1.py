...
def _timeout_handler(_signum, _frame):
    raise TimeoutError(
        "B-row time-Noether background identity exceeded 90 seconds"
    )
...
def test_B_time_noether_background_identity():
    signal.signal(signal.SIGALRM, _timeout_handler)
    signal.alarm(90)

    try:
        assert complete.FIELD_ORDER == total.VARIABLES
        assert "B" in complete.FIELD_ORDER
        ...
        T, Tp, Tpp, Tppp = sp.symbols(
            "B_time_parameter "
            "B_time_parameter_prime "
            "B_time_parameter_double_prime "
            "B_time_parameter_triple_prime"
        )
        ...
        expected_T = (
            ...
            * (
                ...
                - 4 * background["lapse_Friedmann"]
            )
        )

        identity_residual = sp.cancel(
            coefficients["T"] - expected_T
        )
        ...
        print(
            "B_TIME_NOETHER_IDENTITY := "
            "C_B_T = k2/3*("
            "a*E_spatial_trace - 4*E_lapse_Friedmann)"
        )
        print(
            "B_TIME_NOETHER_IDENTITY_RESIDUAL := "
            f"{identity_residual}"
        )
        print(
            "B_HIGHER_PARAMETER_JET_RESIDUALS := "
        ...
        assert all(
            residual == 0
            for residual in higher_parameter_residuals.values()
        )
