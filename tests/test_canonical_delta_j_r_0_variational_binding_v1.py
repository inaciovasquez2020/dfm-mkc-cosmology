...
def _timeout_handler(_signum, _frame):
    raise TimeoutError(
        "canonical delta_J_r_0 variational binding exceeded 120 seconds"
    )


def test_canonical_delta_j_r_0_row_is_direct_original_action_variation():
    signal.signal(signal.SIGALRM, _timeout_handler)
    signal.alarm(120)

    try:
        assert complete.FIELD_ORDER == total.VARIABLES
        assert "delta_J_r_0" in complete.FIELD_ORDER
        ...
        t, s, c, sn = sp.symbols(
            "delta_J_r_0_test_parameter "
            "delta_J_r_0_general_parameter "
            "delta_J_r_0_cosine "
            "delta_J_r_0_sine"
        )

        u = sp.Symbol(
            "delta_J_r_0_test_amplitude"
        )
        ...
        independently_linearized_radiation_j0_euler = sp.expand(
            sp.diff(
                mixed_density,
                u,
            )
        ...
        canonical_delta_j_r_0_row = sp.expand(
            sp.Add(
                *(
        ...
        difference = sp.expand(
            canonical_delta_j_r_0_row
            - independently_linearized_radiation_j0_euler
        )
        ...
        print(
            "DELTA_J_R_0_VARIATIONAL_SOURCE := "
            "independent mixed variation of the unexpanded "
            "radiation Schutz-Sorkin action"
        )
        print(
            "DELTA_J_R_0_ROW_NONZERO_COEFFICIENT_RESIDUALS := "
            f"{nonzero_coefficients}"
        )
        print(
            "DELTA_J_R_0_ROW_PARAMETER_FREE_RESIDUAL := "
