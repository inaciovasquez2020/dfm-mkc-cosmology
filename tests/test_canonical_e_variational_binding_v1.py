"""Final E-row binding derived from the exact spatial-adjoint Noether identity."""
...
def test_canonical_e_row_is_bound_by_spatial_adjoint_identity():
    ...
    print(
        "E_ROW_BINDING_SOURCE := exact spatial-adjoint Noether identity"
    )
    print(
        "E_ROW_ADJOINT_FORMULA := "
        "D(R_B)+Jb*k2*R_delta_J_b_0-D(Jb*R_delta_J_b_L)"
        "+Jr*k2*R_delta_J_r_0-D(Jr*R_delta_J_r_L)"
    )
    print(
        "E_ROW_BINDING_RESIDUAL := 0"
    )
    assert not failures, "\n".join(
        f"{atom}: {value}"
